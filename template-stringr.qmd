---
title: "ESM week 9 lab stringr woohoo"
author:
  - charlie
format: 
  html:
...
-   `str_to_lower()` # makes lower case
-   `str_to_upper()` # makes upper case
-   `str_to_title()` # makes title case
-   `str_trim()` # cuts out the extra spaces on each side
-   `str_squish()` # cuts out extra spaces on each side and within each phrase

Apply each function individually to the `vec` object. Describe what each function does.

```{r}
vec1 <- str_to_lower(vec)
vec2 <- str_to_upper(vec)
vec3 <- str_to_title(vec)
vec4 <- str_trim(vec)
vec5 <- str_squish(vec)
...
The "Carnivore" also gets counted because it as a "c.rn" pattern.
